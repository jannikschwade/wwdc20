...
/**
 Class to display two images in an interactiv splitview.
 - Authors: ioramashvili (2017) and Jannik Schwade (2020)
 - Copyright: Released publicly under MIT Licence (edited)
 
 [Original Code on GitHub]: https://github.com/ioramashvili/BeforeAfterSlider/blob/master/BeforeAfterSlider/BeforeAfterView.swift
 Find the code: [Original Code on GitHub]
 */
...
public class BeforeAfterView: UIView {
    ...
    fileprivate lazy var thumbNob: UIView = {
        let v = UIView()
        v.backgroundColor = UIColor.CustomColor.primaryGreen
        v.translatesAutoresizingMaskIntoConstraints = false
        v.clipsToBounds = true
        return v
    }()
    
    fileprivate lazy var thumbLine: UIView = {
        let line = UIView()
...
extension BeforeAfterView {
    fileprivate func initialize() {
        ...
        thumbWrapper.addSubview(thumbNob)
        thumbWrapper.addSubview(thumbLine)
        ...
        NSLayoutConstraint.activate([
            ...
            thumbWrapper.widthAnchor.constraint(equalToConstant: 30)
        ])
        ...
        NSLayoutConstraint.activate([
            thumbNob.centerXAnchor.constraint(equalTo: thumbWrapper.centerXAnchor, constant: 0),
            thumbNob.centerYAnchor.constraint(equalTo: thumbWrapper.centerYAnchor, constant: 0),
            thumbNob.widthAnchor.constraint(equalToConstant: 8),
            thumbNob.heightAnchor.constraint(equalToConstant: 8)
        ])
        
        NSLayoutConstraint.activate([
            thumbLine.centerXAnchor.constraint(equalTo: thumbWrapper.centerXAnchor, constant: 0),
            thumbLine.centerYAnchor.constraint(equalTo: thumbWrapper.centerYAnchor, constant: 0),
            thumbLine.widthAnchor.constraint(equalToConstant: 3),
            thumbLine.heightAnchor.constraint(equalTo: image1Wrapper.heightAnchor, multiplier: 1)
        ])
        
        leading.constant = frame.width / 2
        
        thumb.layer.cornerRadius = 15
        thumbNob.layer.cornerRadius = 4
